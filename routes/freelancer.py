...
def register_routes(api_ns):
    ...
    # API Models
    profile_model = api_ns.model('FreelancerProfile', {
    ...
    project_model = api_ns.model('Project', {
        'id': fields.Integer(readonly=True),
        'title': fields.String(),
    ...
    application_model = api_ns.model('Application', {
        'id': fields.Integer(readonly=True),
    ...
    class FreelancerProfile(Resource):
        @api_ns.marshal_with(profile_model)
    ...
    class FreelancerProjects(Resource):
        ...
        @api_ns.marshal_list_with(project_model, envelope='data')
        @require_role('freelancer')
        def get(self):
            ...
            if args['status']:
                if args['status'] == 'posted':
                    # Available projects
                    query = query.filter(Project.status.in_(['posted', 'open']), Project.freelancer_id.is_(None))
                ...
                query = query.filter(
                    (Project.status.in_(['posted', 'open']) & Project.freelancer_id.is_(None)) |
    ...
    class FreelancerApplications(Resource):
        ...
        @api_ns.expect(parser)
        @api_ns.marshal_list_with(application_model, envelope='data')
        @require_role('freelancer')
        def get(self):
            args = self.parser.parse_args()
            freelancer = request.user
    ...
    class ApplyToProject(Resource):
        ...
        def post(self, project_id):
            ...
            project = Project.query.filter(Project.id == project_id, Project.status.in_(['posted', 'open'])).first()
            if not project:
